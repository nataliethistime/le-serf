...
class BuildShips {
  ...
  determineToBuild (quantity, sy, sp) {
    if (this.options.topoff) {
      return this.getExistingShipsCount(sp).then((existing) => {
        return this.fixQuantity(this.options.quantity - existing, sy).then((q) => {
          return [q, existing]
        })
      })
    } else {
      return this.fixQuantity(this.options.quantity, sy).then((thing) => [thing])
    }
  }
  ...
  handleColony (colony, buildings) {
    return this.findShipBuildings(colony, buildings).spread((sy, sp) => {
      return this.determineToBuild(this.options.quantity, sy, sp).spread((toBuild, existing) => {
        if (this.options.quantity > toBuild) {
          ...
          if (this.options.topoff) {
            ...
            let existingPlural = util.handlePlurality(existing, this.options.type)
            log.info(`${existing} ${existingPlural} already there, ${msg}`)
          } else {
