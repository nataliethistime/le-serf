...
class MakeHalls {
  ...
  getArchaeology () {
    return new Promise((resolve, reject) => {
      return lacuna.empire.findPlanet(this.options.planet)
        .then((result) => {
          this.colony = result
          return lacuna.body
  ...
  makeRecipe (recipe) {
    return new Promise((resolve, reject) => {
      ...
      if (!quantity > 0) {
        ...
        lacuna.buildings.archaeology.assembleGlyphs(params).then(() => {
          resolve(quantity)
        }).catch(reject)
      }
    })
  ...
  run () {
    return new Promise((resolve, reject) => {
      this.getArchaeology().then(() => {
        return lacuna.buildings.archaeology.getInventory(this.archaeology.id)
      }).then((inventory) => {
        this.inventory = inventory
        return this.makeHalls()
      }).then((numMade) => {
        if (numMade > 0) {
          let plural = util.handlePlurality(numMade, 'Hall')
          resolve(`Made ${numMade} ${plural} on ${this.colony.name}`)
        } else {
