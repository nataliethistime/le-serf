...
class PushGlyphs {
  ...
  run () {
    return new Promise((resolve, reject) => {
      Promise.join(
        lacuna.empire.findPlanets(this.options.from, this.options.to),
        lacuna.empire.findPlanet(this.options.to)
      ).spread((fromColonies, to) => {
        return lacuna.empire.eachPlanet(fromColonies, (colony, buildings) => {
          return this.pushGlyphs(colony, to, buildings)
        })
      }).then(() => {
        let plural = util.handlePlurality(this.glyphsPushed, 'glyph') > 1
          ? 'glyphs'
          : 'glyph'
