...
class UpgradeBuildings {
  ...
  checkPossibleUpgrade (bd) {
    return new Promise((resolve, reject) => {
      let b = this.lacuna.building.generic(bd.url)
  ...
  handleColony (colony) {
    ...
    return this.lacuna.body.buildings(colony.id).then((buildings) => {
      let toUpgrade = _.chain(buildings)
        .each((b) => {
          if (b.pending_build) {
            this.upgradeEndDates.push(b.pending_build.end)
          }
        })
        .map((b) => {
  ...
  run () {
    return new Promise((resolve, reject) => {
      return this.lacuna.authenticate().then(() => {
        ...
        return empireUtils.handlePlanetArg(this.lacuna, this.options.planet, this.options.skip)
      }).then((colonies) => {
